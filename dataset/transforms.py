...
#----------------------data augment-------------------------------------------
class Resize:
    def __init__(self, scale,scale_depth):
        # self.shape = [shape, shape, shape] if isinstance(shape, int) else shape
        self.scale = scale
        self.scale_depth = scale_depth

    def __call__(self, img, mask):
#        self.scale=self.scale/img.shape[2]
#        if self.scale_depth is not None:
#            self.scale_depth=self.scale_depth/img.shape[1]    
#        else:
#            self.scale_depth=1
        #print(self.scale,' ',self.scale_depth)
        img, mask = img.unsqueeze(0), mask.unsqueeze(0).float()
        #img = F.interpolate(img, scale_factor=(self.scale_depth,self.scale,self.scale),mode='trilinear', align_corners=False, recompute_scale_factor=True)
        #mask = F.interpolate(mask, scale_factor=(self.scale_depth,self.scale,self.scale), mode="nearest", recompute_scale_factor=True)
        img = F.interpolate(img, size=(self.scale_depth,self.scale,self.scale),mode='trilinear', align_corners=False, recompute_scale_factor=False)
        mask = F.interpolate(mask, size=(self.scale_depth,self.scale,self.scale), mode="nearest", recompute_scale_factor=False)
